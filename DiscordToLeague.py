...
davemessage = ["That's an interesting question, why do you ask?",
               "I don't see why not.",
               "It is what it is.",
...
async def on_message(message):
    ...
    if message.content.startswith("Dave, ") or message.content.startswith("dave, "):
        if usermessage[1] == "do":
            await channel.send(random.choice(["Yes", "No", "Maybe"]))
        elif "mads" in usermessage:
            await channel.send("Mads does not have my soul and has not harmed me")
        elif "ramz" in usermessage:
            await channel.send("Ramz is 12 and a half and graduated at the age of 8")
        else:
            await channel.send(random.choice(davemessage))
...
(command name) - lists information about the command""")
            elif usermessage[1] == "champs" or usermessage[1] == "Champs":
...
info freechamps - See what champions are free this week
""")
            else:
                await channel.send("""**Syntax:**
!help
...
!info mastery (summonername)""")
        ...
        elif usermessage[1] == "Freechamps" or usermessage[1] == "freechamps":
            info_bot_message = "**Free Champs** \n"
            for Champid in requestFreeChamps()["freeChampionIds"]:
                info_bot_message += dbrf.find_champ(Champid) + "\n"
        else:
            try:
